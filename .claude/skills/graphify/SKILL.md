---
name: graphify
description: "Codebase knowledge graph (query/path/explain). In arbitrator: use only for unknown multi-file code paths — not for exchange/account/runtime symbol data. See .cursor/rules/graphify.mdc."
---

# /graphify

**This project:** graphify maps code structure only. For balances, orders, PnL use `scripts/inspect_exchanges.py` and `scripts/trade_report.py`. Run query/path/explain when the implementation location is unknown; skip when the user named a file or line.
