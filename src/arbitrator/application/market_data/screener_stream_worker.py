...
from arbitrator.application.market_data.multi_exchange_watcher import MultiExchangeWatcher
...
class ScreenerStreamWorker:
    ...
    async def _async_main(self) -> None:
        ...
        try:
            ...
            self._set_symbols_by_exchange(symbols_by_exchange)
    ...
    def read_symbols_by_exchange(self) -> dict[str, list[str]]:
        with self._lock:
            return {ex: list(syms) for ex, syms in self._symbols_by_exchange.items()}

    def _set_symbols_by_exchange(self, symbols_by_exchange: Mapping[str, Sequence[str]]) -> None:
        with self._lock:
            self._symbols_by_exchange = {
