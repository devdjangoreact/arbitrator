...
class ExchangeOrdersService:
    ...
    def _closed_pair_to_group(
        ...
        short_notional = (
            (short.contracts or 0.0)
            * (short.contract_size if short.contract_size > 0 else 1.0)
            * (short.entry_price or short_exit or 0.0)
        )
        long_notional = (
            (long.contracts or 0.0)
            * (long.contract_size if long.contract_size > 0 else 1.0)
            * (long.entry_price or long_exit or 0.0)
        )
    ...
    def _closed_leg_to_single(self, leg: ClosedPositionLeg) -> dict[str, object]:
        ...
        csize = leg.contract_size if leg.contract_size > 0 else 1.0
        coins = (leg.contracts or 0.0) * csize
        ref_price = leg.entry_price or leg.exit_price or 0.0
        volume = round(coins * ref_price, 2) or None
