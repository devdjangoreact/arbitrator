...
## Сценарій 7: Backend persistence (FR-024) + Close card (FR-025)

1. Запустити screener, відкрити картку через Fast Trade
2. Закрити вкладку браузера (не натискаючи Stop)
3. Почекати 15 с, відкрити `http://localhost:8000` знову
4. **Очікуваний результат**: картка присутня, дані актуальні, screener не зупинявся
5. Натиснути × на картці
6. **Очікуваний результат**: картка зникає після наступного пушу; у логах бекенду — рядки про закриття позицій

---

## Сценарій 8: Фільтри Candle Interval + Price Deviation

1. У фільтрах виставити Candle Interval = 30 (с), натиснути Apply
2. Перевірити що таблиця оновилась (рядки можуть змінитись)
3. Виставити Price Deviation = 1.0%, Apply
4. **Очікуваний результат**: рядки де відхилення ціни < 1% зникли зі списку
5. Скинути Price Deviation = 0, Apply → всі рядки повернулись
